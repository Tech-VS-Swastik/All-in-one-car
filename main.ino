  /*
 * LKK = LazyKillerKing
 * SMA = SuperMasterAgent
 * SA = Swastik Arors
 * GA = Gopal Arora - PAPA
 * PA = Pulkit Arora - BHAIYA
 * AK = Arsh Khandelwal
 * PS = Priyanshu Singla
 * Idea BY SA
 * Confirmed BY LKK
 * Project Name (SMA) advised BY BHAIYA
 * Project Name BY LKK SA
 * Help By PAPA and BHAIYA
 * Friends - AK and PS
 * Confirmed By BHAIYA
 * Written BY LKK SA
 * Copyright action from LKK SA©
 * Assebled By LKK SA
 * Completed By LKK SA
 * if 11 dont work put A5
 */
...
//===============================================================================    
else if(bt_ir_data == 6){turnLeft();  delay(300);  bt_ir_data = 5;}
else if(bt_ir_data == 7){turnRight(); delay(300);  bt_ir_data = 5;}
}
...
long IRremote_data(){
     if(results.value==0x1FE58A7 || results.value==0x2D607981){IR_data=1;}  
else if(results.value==0x1FEA05F || results.value==0x3E3D6F9){IR_data=2;} 
else if(results.value==0x1FE807F || results.value==0x69C6FA7D){IR_data=3;} 
else if(results.value==0x592FA519 || results.value==0x1FE30CF || results.value==0x1FEC03F){IR_data=4;} 
else if(results.value==0x1FE40BF || results.value==0xDE4001F || results.value==0xEF3B295B || results.value==0x1FE48B7){IR_data=5;} 
else if(results.value==0x1FED827 || results.value==0x87EAA93D){IR_data=8;} 
else if(results.value==0x1FE50AF){IR_data=9;} 
else if(results.value==0x1FEF807){IR_data=10;} 
return IR_data;
}
